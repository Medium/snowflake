...
import {
  trackIds, milestones, tracks, categoryColorScale,
} from '../data/constants';
import type { TrackId, Milestone, MilestoneMap } from '../data/constants';
              ...
              return (
                <g key={trackId} transform={`rotate(${(i * 360) / trackIds.length})`}>
                  {arcMilestones.map((milestone) => {
                    ...
                    const fill = isMet ? categoryColorScale(tracks[trackId].category) : undefined;
                    return (
                      <path
                        key={milestone}
